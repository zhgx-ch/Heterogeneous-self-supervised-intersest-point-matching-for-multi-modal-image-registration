...
class Realimage(BaseDataset):
    ...
    def _init_dataset(self, **config):
        base_path = Path(DATA_PATH, 'real_image/') #构建一个路径对象
        image_paths = sorted(os.listdir(base_path),key = lambda i:int(i.split("_")[-2]))   #生成文件夹中的所有目录路径，并将路径格式转为列表格式
#         if config['truncate']:
#             image_paths = image_paths[:config['truncate']]
        ...
        if config['labels1']:
            label_paths = []
            for n in names:
                p = Path(EXPER_PATH, config['labels1'], '{}.npz'.format(n))
