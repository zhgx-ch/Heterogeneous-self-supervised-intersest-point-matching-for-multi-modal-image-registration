...
class Optical(BaseDataset):
    ...
    def _init_dataset(self, **config):
        base_path = Path(DATA_PATH, 'optical/') #构建一个路径对象
        image_paths = sorted(os.listdir(base_path),key = lambda i:int(i.split(".")[0])) #生成文件夹中的所有目录路径，并将路径格式转为列表格式
#         if config['truncate']:
