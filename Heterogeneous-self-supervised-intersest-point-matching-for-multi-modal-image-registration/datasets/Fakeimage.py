...
class Fakeimage(BaseDataset):
    ...
    def _init_dataset(self, **config):
        ...
        base_path = Path(DATA_PATH, 'fake_image/') #构建一个路径对象
#         import pdb
#         pdb.set_trace() 
        
        image_paths = sorted(os.listdir(base_path),key = lambda i:int(i.split("_")[-2]))   #生成文件夹中的所有目录                                                               #路径，并将路径格式转为列表格式
